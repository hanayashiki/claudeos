...
# An assignment's value is expanded but not split into fields or globbed
# (POSIX 2.9.1). Split, the words after the first ran as a command.
check "assigned substitution"  "a  b*"   "$(v=$(printf 'a  b*'); echo "$v")"
check "assigned parameter"     "p  q"    "$(x='p  q'; v=$x; echo "$v")"
check "assignment not globbed" "x*"      "$(mkdir -p /tmp/ag; cd /tmp/ag; touch v=xa v=xb; v=x*; echo "$v"; cd /root; rm -rf /tmp/ag)"
check "prefix substitution"    "a  b"    "$(v=$(printf 'a  b') sh -c 'echo "$v"')"
check "export substitution"    "a  b"    "$(export v=$(printf 'a  b'); sh -c 'echo "$v"')"
check "local substitution"     "a  b"    "$(f() { local v=$(printf 'a  b'); echo "$v"; }; f)"
check "argument still split"   "2"       "$(set -- $(printf 'a  b'); echo $#)"
